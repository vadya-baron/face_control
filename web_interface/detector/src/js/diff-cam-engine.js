var DiffCamEngine = (function() {
	var stream;					// трансляция, полученная с веб-камеры
	var video;					// показывает поток
	var captureCanvas;			// внутренний холст для захвата полных изображений из видео
	var captureContext;			// контекст для захвата холста
	var diffCanvas;				// внутренний холст для различных уменьшенных снимков
	var diffContext;			// контекст для diff canvas
	var motionCanvas;			// получает обработанные изображения с различиями
	var motionContext;			// контекст для движущегося холста

	var initSuccessCallback;	// вызывается, когда инициализация завершается успешно
	var initErrorCallback;		// вызывается при сбое инициализации
	var startCompleteCallback;	// вызывается по завершении запуска
	var captureCallback;		// вызывается, когда изображение было захвачено и изменено

	var captureInterval;		// интервал для непрерывных захватов
	var captureIntervalTime;	// время между захватами, в мс
	var captureWidth;			// полная ширина захваченного изображения
	var captureHeight;			// полная высота захваченного изображения
	var diffWidth;				// уменьшенная ширина для изменения/перемещения
	var diffHeight;				// уменьшенная высота для изменения/перемещения
	var isReadyToDiff;			// был ли сделан предыдущий захват для сравнения?
	var pixelDiffThreshold;		// минимум для того, чтобы пиксель считался значимым
	var scoreThreshold;			// минимум для того, чтобы изображение считалось значимым
	var includeMotionBox;		// флажок для вычисления и рисования ограничивающей рамки движения
	var includeMotionPixels;	// флаг для создания объекта, обозначающего пиксели с движением
